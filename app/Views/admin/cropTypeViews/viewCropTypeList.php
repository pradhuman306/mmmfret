        ...
        <div class="card card-info">
            ...
            <div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>
					...
					<table id="tableOfCroptypes" class="table table-striped table-hover using-exportable-data-table" style="width: 100%;">
						<thead>
							<tr>
           						<th><?= lang('CropTypes.cropType') ?></th>
								<th><?= lang('CropTypes.variety') ?></th>
                				<th class="text-nowrap" style="width: 1%;"><?= lang('Basic.global.Action') ?></th>
							</tr>
						</thead>
						<tbody>
						<?php foreach ($cropTypeList as $item ) : 
                            $item->created_at = $timezoneConverter->convertToUserTimezone($item->id, $userId,'created_at');
                            $item->updated_at = $timezoneConverter->convertToUserTimezone($item->id, $userId,'updated_at');
                            ?>
							<tr>
								<td class="align-middle crop-type-name">
                                    <?= empty($item->crop_type) || strlen($item->crop_type) < 51 ? esc($item->crop_type) : character_limiter(esc($item->crop_type), 50) ?>
								</td>
								<td class="align-middle">
									<?= empty($item->variety) || strlen($item->variety) < 51 ? esc($item->variety) : character_limiter(esc($item->variety), 50)   ?>
								</td>
                                    <td class="align-middle text-center text-nowrap">
									<?=anchor(route_to('editCropType', $item->id), '<i class="bi bi-pencil-square"></i>', ['class'=>'btn btn-sm btn-outline-success btn-edit me-1',  'data-id'=>$item->id,]); ?> 
									<?=anchor('#confirm2delete', '<i class="bi bi-trash"></i>', ['class'=>'btn btn-sm btn-outline-danger btn-delete ms-1', 'data-href'=>route_to('deleteCropType', $item->id)]); ?>
								</td>
                                </tr>
						<?php endforeach; ?>
						</tbody>
...
const lastColNr2 = $(".using-exportable-data-table").find("tr:first th").length - 1;
    ...
    const itemName = $(this).closest('tr').find('.crop-type-name').text().trim(); // Now using the new class for crop type

    Swal.fire({
        title: `Are you sure you want to delete "${itemName}"?`, // itemName is the crop type
        icon: 'warning',
