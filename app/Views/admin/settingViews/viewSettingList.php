    ...
    <div class="col-md-12">
        ...
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><?=lang('Settings.settingList') ?></h3>
           </div><!--//.card-header -->
            <div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>

					<table id="tableOfSettings" class="table table-striped table-hover using-exportable-data-table" style="width: 100%;">
						<thead>
							<tr>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
								<th><?= lang('Settings.id') ?></th>
								<th><?= lang('Settings.description') ?></th>
								<th><?= lang('Settings.companyId') ?></th>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
							</tr>
						</thead>
						<tbody>
						<?php foreach ($settingList as $item ) : ?>
							<tr>
								<td class="align-middle text-center text-nowrap">
									<?=anchor(route_to('editSetting', $item->id), '<i class="bi bi-pencil-square"></i>', ['class'=>'btn btn-sm btn-warning btn-edit me-1',  'data-id'=>$item->id,]); ?> 
									<?=anchor('#confirm2delete', '<i class="bi bi-trash"></i>', ['class'=>'btn btn-sm btn-danger btn-delete ms-1', 'data-href'=>route_to('deleteSetting', $item->id)]); ?>
								</td>
								<td class="align-middle text-center">
									<?=$item->id ?>
								</td>
								<td class="align-middle">
									<?= empty($item->description) || strlen($item->description) < 51 ? esc($item->description) : character_limiter(esc($item->description), 50)   ?>
								</td>
								<td class="align-middle">
									<?= esc($item->company_id) ?>
								</td>
            ...
            <div class="card-footer">
				<?=anchor(route_to('newSetting'), lang('Basic.global.addNew').' '.lang('Settings.setting'), ['class'=>'btn btn-primary float-end']); ?>
            </div><!--//.card-footer -->
        </div><!--//.card -->
    </div><!--//.col -->
</div><!--//.row -->
...
<?=$this->section('additionalInlineJs') ?>
    ...
    $(document).on('click', '.btn-delete', function(e) {
            ...
            Swal.fire({
                title: "<?= lang('Basic.global.sweet.sureToDeleteTitle', [lang('Settings.setting')]) ?>",
