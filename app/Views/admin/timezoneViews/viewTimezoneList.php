...
<?=$this->section('additionalInlineJs') ?>
            ...
            const lastColNr = $('#tableOfTimezones').find("tr:first th").length - 1;
            ...
            theTable = $('#tableOfTimezones').DataTable({
                createdRow: function(row, data, dataIndex) {
            // Add a custom class to the 'type' column
            var typeCell = $(row).find('td:eq(0)'); // Assuming 'type' is the first column (index 0)
                typeCell.addClass('timezone-name'); // Replace 'custom-class-name' with your desired class
            },
        ...
        extend: 'pdfHtml5',
            ...
            exportOptions: {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= base_url().route_to('dataTableOfTimezones') ?>',
                    method: 'POST',
...
$(document).on('click', '.btn-edit', function(e) {
        window.location.href = `<?= base_url().route_to('timezoneList') ?>/${$(this).attr('data-id')}/edit`;
    });
    
$(document).on('click', '.btn-delete', function(e) {
    const itemName = $(this).closest('tr').find('.timezone-name').text().trim(); // Now using the new class for Timezone
        Swal.fire({
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= base_url().route_to('timezoneList') ?>/${dataId}`,
                    method: 'DELETE',
