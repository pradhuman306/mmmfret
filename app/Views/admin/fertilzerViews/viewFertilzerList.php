    ...
    <div class="col-md-12">
        ...
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><?=lang('Fertilzers.fertilzerList') ?></h3>
                <?=anchor(route_to('newFertilzer'), lang('Basic.global.addNew').' '.lang('Fertilzers.fertilzer'), ['class'=>'btn btn-success float-end']); ?>
           </div><!--//.card-header -->
            <div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>

					<table id="tableOfFertilzers" class="table table-striped table-hover" style="width: 100%;">
						<thead>
							<tr>
    							<th><?= lang('Fertilzers.fertilzer') ?></th>
								<th><?= lang('Fertilzers.symbol') ?></th>
								<th><?= lang('Fertilzers.nitrogen') ?></th>
								<th><?= lang('Fertilzers.phosphorus') ?></th>
								<th><?= lang('Fertilzers.potassium') ?></th>
								<th><?= lang('Fertilzers.calcium') ?></th>
								<th><?= lang('Fertilzers.sulfur') ?></th>
								<th><?= lang('Fertilzers.magnesium') ?></th>
								<th><?= lang('Fertilzers.maganense') ?></th>
								<th><?= lang('Fertilzers.boron') ?></th>
...
<?=$this->section('additionalInlineJs') ?>
            ...
            const lastColNr = $('#tableOfFertilzers').find("tr:first th").length - 1;
            ...
            theTable = $('#tableOfFertilzers').DataTable({
                processing: true,
                serverSide: true,
                autoWidth: true,
                responsive: true,
        ...
        extend: 'pdfHtml5',
            ...
            exportOptions: {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= base_url().route_to('dataTableOfFertilzers') ?>',
                    method: 'POST',
...
$(document).on('click', '.btn-edit', function(e) {
        window.location.href = `<?= base_url().route_to('fertilzerList') ?>/${$(this).attr('data-id')}/edit`;
    });
...
$(document).on('click', '.btn-delete', function(e) {
    const itemName = $(this).closest('tr').find('td:first-child').text().trim();
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= base_url().route_to('fertilzerList') ?>/${dataId}`,
                    method: 'DELETE',
