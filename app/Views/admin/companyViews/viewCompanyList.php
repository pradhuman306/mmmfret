...
<?=$this->section('additionalInlineJs') ?>
            ...
            const lastColNr = $('#tableOfCompanies').find("tr:first th").length - 1;
            ...
            theTable = $('#tableOfCompanies').DataTable({
                processing: true,
                serverSide: true,
                autoWidth: true,
                responsive: true,
		...
		"buttons": [
			'copy', 'csv', 'excel', 'print', {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= base_url().route_to('dataTableOfCompanies') ?>',
                    method: 'POST',
...
$(document).on('click', '.btn-delete', function(e) {
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= base_url().route_to('companyList') ?>/${dataId}`,
                    method: 'DELETE',
