    ...
    <div class="col-md-12">
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><?=lang('Categories.categoryList') ?></h3>
                <?=anchor(route_to('newCategory'), lang('Basic.global.addNew').' '.lang('Categories.category'), ['class'=>'btn btn-success float-end']); ?>
           </div><!--//.card-header -->
            <div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>

					<table id="tableOfCategories" class="table table-striped table-hover" style="width: 100%;">
						<thead>
							<tr>
...
<?=$this->section('additionalInlineJs') ?>
            ...
            const lastColNr = $('#tableOfCategories').find("tr:first th").length - 1;
            ...
            theTable = $('#tableOfCategories').DataTable({
                processing: true,
                serverSide: true,
                autoWidth: true,
                responsive: true,
        ...
        extend: 'pdfHtml5',
            ...
            exportOptions: {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= base_url().route_to('dataTableOfCategories') ?>',
                    method: 'POST',
...
$(document).on('click', '.btn-edit', function(e) {
        window.location.href = `<?= base_url().route_to('categoryList') ?>/${$(this).attr('data-id')}/edit`;
    });
...
$(document).on('click', '.btn-delete', function(e) {
    const itemName = $(this).closest('tr').find('td:first-child').text().trim();
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= base_url().route_to('categoryList') ?>/${dataId}`,
                    method: 'DELETE',
