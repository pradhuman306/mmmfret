    ...
    <div class="col-md-12">
        ...
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><?=lang('PricelistTemplates.priceListTemplateList') ?></h3>
           </div><!--//.card-header -->
            <div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>

					<table id="tableOfPricelisttemplates" class="table table-striped table-hover" style="width: 100%;">
						<thead>
							<tr>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
								<th><?=lang('PricelistTemplates.id')?></th>
								<th><?= lang('PricelistTemplates.details') ?></th>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
							</tr>
						</thead>
						<tbody>
            ...
            <div class="card-footer">
				<?=anchor(route_to('newPriceListTemplate'), lang('Basic.global.addNew').' '.lang('PricelistTemplates.priceListTemplate'), ['class'=>'btn btn-primary float-end']); ?>
            </div><!--//.card-footer -->
        </div><!--//.card -->
    </div><!--//.col -->
</div><!--//.row -->
...
<?=$this->section('additionalInlineJs') ?>
            ...
            const lastColNr = $('#tableOfPricelisttemplates').find("tr:first th").length - 1;
            ...
            theTable = $('#tableOfPricelisttemplates').DataTable({
                processing: true,
                serverSide: true,
                autoWidth: true,
                responsive: true,
		...
		"buttons": [
			'copy', 'csv', 'excel', 'print', {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= base_url().route_to('dataTableOfPriceListTemplates') ?>',
                    method: 'POST',
...
$(document).on('click', '.btn-edit', function(e) {
        window.location.href = `<?= base_url().route_to('priceListTemplateList') ?>/${$(this).attr('data-id')}/edit`;
    });
    
$(document).on('click', '.btn-delete', function(e) {
        Swal.fire({
            title: '<?= lang('Basic.global.sweet.sureToDeleteTitle', [mb_strtolower(lang('PricelistTemplates.price list template'))]) ?>',
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= base_url().route_to('priceListTemplateList') ?>/${dataId}`,
                    method: 'DELETE',
