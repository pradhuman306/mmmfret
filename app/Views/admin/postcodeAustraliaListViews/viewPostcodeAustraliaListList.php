    ...
    <div class="col-md-12">
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><?=lang('PostcodeAustraliaLists.postcodeAustraliaListList') ?></h3>
                <?=anchor(route_to('newPostcodeAustraliaList'), lang('Basic.global.addNew').' '.lang('PostcodeAustraliaLists.postcodeAustraliaList'), ['class'=>'btn btn-success float-end']); ?>
           </div><!--//.card-header -->
            <div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>
                <br>

					<table id="tableOfPostcodeaustralialists" class="table table-striped table-hover" style="width: 100%;">
						<thead>
							<tr>
								<th><?= lang('PostcodeAustraliaLists.suburb') ?></th>
								<th><?= lang('Timezones.timezone') ?></th>
								<th><?= lang('States.state') ?></th>
								<th><?= lang('PostcodeAustraliaLists.postcode') ?></th>
								<th><?= lang('PostcodeAustraliaLists.country') ?></th>
...
<?=$this->section('additionalInlineJs') ?>
            ...
            const lastColNr = $('#tableOfPostcodeaustralialists').find("tr:first th").length - 1;
            ...
            theTable = $('#tableOfPostcodeaustralialists').DataTable({
                createdRow: function(row, data, dataIndex) {
                // Add a custom class to the 'type' column
                var typeCell = $(row).find('td:eq(0)'); // Assuming 'type' is the first column (index 0)
                typeCell.addClass('suburb-name'); // Replace 'custom-class-name' with your desired class
        },
                processing: true,
                serverSide: true,
                autoWidth: true,
        ...
        extend: 'pdfHtml5',
            ...
            exportOptions: {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= base_url().route_to('dataTableOfPostcodeAustraliaLists') ?>',
                    method: 'POST',
...
$(document).on('click', '.btn-edit', function(e) {
        window.location.href = `<?= base_url().route_to('postcodeAustraliaListList') ?>/${$(this).attr('data-id')}/edit`;
    });
    
$(document).on('click', '.btn-delete', function(e) {
    const itemName = $(this).closest('tr').find('.suburb-name').text().trim(); // Now using the new class for Suburb
        Swal.fire({
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= base_url().route_to('postcodeAustraliaListList') ?>/${dataId}`,
                    method: 'DELETE',
