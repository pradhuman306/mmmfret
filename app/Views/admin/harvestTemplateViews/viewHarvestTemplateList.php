    ...
    <div class="col-md-12">
        ...
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><?=lang('HarvestTemplates.harvestTemplateList') ?></h3>
           </div><!--//.card-header -->
            <div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>

					<table id="tableOfHarvesttemplates" class="table table-striped table-hover" style="width: 100%;">
						<thead>
							<tr>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
								<th><?=lang('HarvestTemplates.id')?></th>
								<th><?= lang('HarvestTemplates.description') ?></th>
								<th class="text-nowrap"><?= lang('Basic.global.Action') ?></th>
							</tr>
						</thead>
						<tbody>
            ...
            <div class="card-footer">
				<?=anchor(route_to('newHarvestTemplate'), lang('Basic.global.addNew').' '.lang('HarvestTemplates.harvestTemplate'), ['class'=>'btn btn-primary float-end']); ?>
            </div><!--//.card-footer -->
        </div><!--//.card -->
    </div><!--//.col -->
</div><!--//.row -->
...
<?=$this->section('additionalInlineJs') ?>
            ...
            const lastColNr = $('#tableOfHarvesttemplates').find("tr:first th").length - 1;
            ...
            theTable = $('#tableOfHarvesttemplates').DataTable({
                processing: true,
                serverSide: true,
                autoWidth: true,
                responsive: true,
		...
		"buttons": [
			'copy', 'csv', 'excel', 'print', {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= base_url().route_to('dataTableOfHarvestTemplates') ?>',
                    method: 'POST',
...
$(document).on('click', '.btn-edit', function(e) {
        window.location.href = `<?= base_url().route_to('harvestTemplateList') ?>/${$(this).attr('data-id')}/edit`;
    });
    
$(document).on('click', '.btn-delete', function(e) {
        Swal.fire({
            title: '<?= lang('Basic.global.sweet.sureToDeleteTitle', [mb_strtolower(lang('HarvestTemplates.harvest template'))]) ?>',
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= base_url().route_to('harvestTemplateList') ?>/${dataId}`,
                    method: 'DELETE',
