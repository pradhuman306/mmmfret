    ...
    <div class="col-md-12">
        ...
        <div class="card card-info">
            <div class="card-header">
                <h3 class="card-title"><?=lang('States.stateList') ?></h3>
                <?=anchor(route_to('newState'), lang('Basic.global.addNew').' '.lang('States.state'), ['class'=>'btn btn-success float-end']); ?>
           </div><!--//.card-header -->
            <div class="card-body">
				<?= view('Themes/_commonPartialsBs/_alertBoxes'); ?>

					<table id="tableOfStates" class="table table-striped table-hover" style="width: 100%;">
						<thead>
							<tr>
...
<?=$this->section('additionalInlineJs') ?>
            ...
            const lastColNr = $('#tableOfStates').find("tr:first th").length - 1;
            const actionBtns = function(data) {
                return `<td class="text-right py-0 align-middle">
                        <div class="btn-group btn-group-sm">
                            <button class="btn btn-sm btn-outline-success btn-edit me-1" data-id="${data.state_id}"><i class="bi bi-pencil-square"></i></button>
                            <button class="btn btn-sm btn-outline-danger btn-delete ms-1" data-id="${data.state_id}"><i class="bi bi-trash"></i></button>
                        </div>
                        </td>`;
            };
            theTable = $('#tableOfStates').DataTable({
                createdRow: function(row, data, dataIndex) {
            // Add a custom class to the 'type' column
            var typeCell = $(row).find('td:eq(0)'); // Assuming 'type' is the first column (index 0)
                typeCell.addClass('state-name'); // Replace 'custom-class-name' with your desired class
            },
                processing: true,
                serverSide: true,
                autoWidth: true,
        ...
        extend: 'pdfHtml5',
            ...
            exportOptions: {
                ...
                ajax : $.fn.dataTable.pipeline( {
                    url: '<?= base_url().route_to('dataTableOfStates') ?>',
                    method: 'POST',
...
$(document).on('click', '.btn-edit', function(e) {
        window.location.href = `<?= base_url().route_to('stateList') ?>/${$(this).attr('data-id')}/edit`;
    });
    
    $(document).on('click', '.btn-delete', function(e) {
    const itemName = $(this).closest('tr').find('.state-name').text().trim(); // Now using the new class for Timezone
        Swal.fire({
            title: `Are you sure you want to delete "${itemName}"?`, // itemName is the statemezone
        ...
        .then((result) => {
            ...
            if (result.value) {
                $.ajax({
                    url: `<?= base_url().route_to('stateList') ?>/${dataId}`,
                    method: 'DELETE',
