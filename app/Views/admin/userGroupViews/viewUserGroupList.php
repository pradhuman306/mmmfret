						...
						<?php foreach ($userGroupList as $item ) : ?>
							<tr>
								<td class="align-middle">
									<?= empty($item->name) || strlen($item->name) < 51 ? esc($item->name) : character_limiter(esc($item->name), 50)   ?>
								</td>
								<td class="align-middle">
								...
								<td class="align-middle text-center text-nowrap">
									<?=anchor(route_to('editUserGroup', $item->id), '<i class="bi bi-pencil-square"></i>', ['class'=>'btn btn-sm btn-outline-success btn-edit me-1',  'data-id'=>$item->id,]); ?> 
									<?=anchor('#confirm2delete', '<i class="bi bi-trash"></i>', ['class'=>'btn btn-sm btn-outline-danger btn-delete ms-1', 'data-href'=>route_to('deleteUserGroup', $item->id)]); ?>
								</td>
							</tr>
...
<?=$this->section('additionalInlineJs') ?>
    ...
    $(document).on('click', '.btn-delete', function(e) {
            ...
            Swal.fire({
                title: "<?= lang('Basic.global.sweet.sureToDeleteTitle', [lang('AuthGroups.user group')]) ?>",
